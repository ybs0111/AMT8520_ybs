...
// 	XgemSendOffSokStatus();
	...
	if( st_handler.mn_xgem_mode == CTL_YES )
	{
		...
		//2017.0626
		XgemLdRbtPickupBufferStart();//9812
		XgemLdRbtPickupBufferEnd();//9813		
...
void AClient_Xgem::OnReceived_XgemAnalysis( CString strRecv )
{
	...
	else if( nCeid == 9812 )
	{
		...
		m_bGemTrayLdPickBufferStart = false;
	}
	else if( nCeid == 9813 )
	{
		st_xgem.m_str_lotid = "";
		...
		m_bGemTrayLdPickBufferEnd = false;
	}
	////////////////////////////////////////////
	else if( nCeid == 9814 )
	{
...
void AClient_Xgem::XgemTrayMoveLdBufferStart()
{
	...
	strFunc.Format("FUNC=CEID,CEID_NUM=9809,REPORT=1,REP_01={CNT=04,[DA_01=01,%s],[DA_02=01,%d],[DA_03=01,%d],[DA_04=01,%d]},", st_lamp.mstr_equip_id, m_nTrayPocketCntBufferStart, m_nPocketCntBufferStart, m_nProductCntBufferStart);
	strHead.Format("HD=%06d,", strFunc.GetLength());
...
void AClient_Xgem::XgemTrayMoveLdBufferEnd()
{
	...
	strFunc.Format("FUNC=CEID,CEID_NUM=9810,REPORT=1,REP_01={CNT=04,[DA_01=01,%s],[DA_02=01,%d],[DA_03=01,%d],[DA_04=01,%d]},", st_lamp.mstr_equip_id, m_nTrayPocketCntBufferEnd, m_nPocketCntBufferEnd, m_nProductCntBufferStart);
	strHead.Format("HD=%06d,", strFunc.GetLength());
...
void AClient_Xgem::XgemLdRbtPickupDvcStart()
{
	...
	strFunc.Format("FUNC=CEID,CEID_NUM=9811,REPORT=1,REP_01={CNT=03,[DA_01=01,%s],[DA_02=01,%d],[DA_03=01,%s]},", st_lamp.mstr_equip_id, m_nTrayNumLdPickDVCStart, m_strTrayLdPickDVCSerailStart);
	strHead.Format("HD=%06d,", strFunc.GetLength());
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//2017.0626
void AClient_Xgem::XgemLdRbtPickupBufferStart()
{
	CString strHead, strFunc, strLotstart;
	if( m_bGemTrayLdPickBufferStart == false )
		return;
	
	if(GetCurrentTime() - m_dwTime_TrayLdPickBufferStart < 0)
	{
		m_dwTime_TrayLdPickBufferStart = GetCurrentTime();
	}
	if( GetCurrentTime() <= m_dwTime_TrayLdPickBufferStart + 5000 )
		return;
	
	if( m_nCntTrayPickBufferStart >= 3 )
	{
		m_nCntTrayPickBufferStart = 0;
		m_bUsable = false;
		
		CString strErr;
		strErr.Format( "Next : XGem DVC Buffer Place Device Start 3회 무응답");
		if ( g_local.GetLocalType() == LOCAL_ENG ) strErr.Format( "Next : XGem DVC Buffer Place Device Start 3count non-response");
		st_msg.mstr_event_msg[0] = strErr;
		::PostMessage(st_handler.hWnd, WM_MAIN_EVENT, CTL_YES, 0);
	...
	strFunc.Format("FUNC=CEID,CEID_NUM=9812,REPORT=1,REP_01={CNT=03,[DA_01=01,%s],[DA_02=01,%d],[DA_03=01,%s]},", st_lamp.mstr_equip_id, m_nTrayNumLdPickBufferStart, m_strTrayLdPickBufferSerailStart);
	strHead.Format("HD=%06d,", strFunc.GetLength());
	
	strLotstart = strHead+strFunc;
	PushSendMsg( (LPCTSTR)strLotstart );
	m_dwTime_TrayLdPickBufferStart = GetCurrentTime();
	m_nCntTrayPickBufferStart++;	
	
	m_bGemTrayLdPickStart = false;
}

void AClient_Xgem::XgemLdRbtPickupBufferEnd()
{
	CString strHead, strFunc, strLotstart;
	if( m_bGemTrayLdPickBufferEnd == false )
		return;
	...
	if( m_nCntTrayPickBufferEnd >= 3 )
	{
		m_nCntTrayPickBufferEnd = 0;
		m_bUsable = false;
		
		CString strErr;
		strErr.Format( "Next : XGem DVC Buffer Pick Device End 3회 무응답");
		if ( g_local.GetLocalType() == LOCAL_ENG ) strErr.Format( "Next : XGem DVC Buffer Place Pick Device End 3count non-response");
		st_msg.mstr_event_msg[0] = strErr;
		::PostMessage(st_handler.hWnd, WM_MAIN_EVENT, CTL_YES, 0);
	...
	strFunc.Format("FUNC=CEID,CEID_NUM=9813,REPORT=1,REP_01={CNT=03,[DA_01=01,%s],[DA_02=01,%d],[DA_03=01,%s]},", st_lamp.mstr_equip_id, m_nTrayNumLdPickBufferEnd, m_strTrayLdPickBufferSerailEnd);
	strHead.Format("HD=%06d,", strFunc.GetLength());
	
	strLotstart = strHead+strFunc;
	PushSendMsg( (LPCTSTR)strLotstart );
	m_dwTime_TrayLdPickBufferStart = GetCurrentTime();
	m_nCntTrayPickBufferEnd++;	
...
void AClient_Xgem::XGemSocketOn()
{
	...
	strFunc.Format("FUNC=CEID,CEID_NUM=9825,REPORT=1,REP_01={CNT=02,[DA_01=01,%s],[DA_02=01,%d]},", st_lamp.mstr_equip_id, m_nSockOnnum );
	strHead.Format("HD=%06d,", strFunc.GetLength());
...
void AClient_Xgem::XGemSocketOff()
{
	...
	strFunc.Format("FUNC=CEID,CEID_NUM=9826,REPORT=1,REP_01={CNT=02,[DA_01=01,%s],[DA_02=01,%d]},", st_lamp.mstr_equip_id, m_nSockOffnum );
	strHead.Format("HD=%06d,", strFunc.GetLength());
