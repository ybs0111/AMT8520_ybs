...
int CComizoaPublic::Set_MotSpeed(int n_SpeedCase, int n_Axis, int n_SpeedMode, double d_work, double d_accel_time, double d_decel_time) 
{
	...
	{	
		if (d_work < 1 || d_work > 2200)	// 1:초당 1cm , max:초당 2000cm 이동 한계 설정
		{
			d_work = 10;
		}
	}
...
{
	int  nRet = BD_ERROR, nRet_l = BD_ERROR, nFlag = 0;
	double d_SpeedVel;
	double dCurrPos = 0;
	long dwMotionDone=0;
...
*/
	...
	if( n_Axis == MOTOR_ROBOT_X || n_Axis == MOTOR_ROBOT_Y )
	{
		if( n_Axis == MOTOR_ROBOT_X )
		{
			dp_Pos[0] = d_Distance;
			dp_Pos[1] = g_comiMgr.Get_MotCurrentPos( MOTOR_ROBOT_Y );

			nRet_l = CTL_Lib.Motor_LinearSafety(M_ROBOT_XY_INDEX,dp_Pos);
			if( nRet_l == BD_ERROR)
			{
				cmmSxStop(n_Axis, false, false);
				return BD_ERROR;
		...
		else if( n_Axis == MOTOR_ROBOT_Y )
		{
			dp_Pos[0] = g_comiMgr.Get_MotCurrentPos( MOTOR_ROBOT_X );
			dp_Pos[1] = d_Distance;
			nRet_l = CTL_Lib.Motor_LinearSafety(M_ROBOT_XY_INDEX,dp_Pos);
			if( nRet_l == BD_ERROR)
			{
	...
	if (nRet != BD_GOOD)
	{
		...
		cmmSxStop(n_Axis, false, false);//2014,0730
...
*/
	...
	{
		double dCurr[2];
		dCurr[0] = COMI.Get_MotCurrentPos(MOTOR_ROBOT_X);
		dCurr[1] = COMI.Get_MotCurrentPos(MOTOR_ROBOT_Y);
		int nMotModex = 0;
		double dRet = 0.0;
		if(n_Axis == MOTOR_ROBOT_X)
		{
//			if(abs(dCurr[0] - d_Distance) < 100 && n_SpeedRatio >= 10)
			//2013,1127//2016.0330
// 			if(abs(dCurr[0] - d_Distance) < 100 && n_SpeedRatio >= 10)
// 			{
// 				d_SpeedVel = d_SpeedVel*0.5;
// 			}
			dRet = XYDistance(dCurr[0], 0.0, d_Distance, 0.0);
			
			if	   ( dRet < 200 )
			{
				d_Speed[0] = d_SpeedVel*0.5; d_Speed[1] = md_spd_vel[n_Axis][1]*0.4; d_Speed[2] = md_spd_vel[n_Axis][2]*0.4;
			}
// 			else if( dRet < 500 )
// 			{
// 				d_Speed[0] = d_SpeedVel*0.5; d_Speed[1] = md_spd_vel[n_Axis][1]*0.5; d_Speed[2] = md_spd_vel[n_Axis][2]*0.5;
// 			}
// 			else if( dRet < 500 )
// 			{
// 				d_Speed[0] = d_SpeedVel*0.85; d_Speed[1] = md_spd_vel[n_Axis][1]*0.6; d_Speed[2] = md_spd_vel[n_Axis][2]*0.6;
// 			}
// 			else if( dRet < 1100 )
// 			{
// 				d_Speed[0] = d_SpeedVel*0.85; d_Speed[1] = md_spd_vel[n_Axis][1]*0.85; d_Speed[2] = md_spd_vel[n_Axis][2]*0.85;
// 			}
			else
			{
		...
		{
//			if(abs(dCurr[1] - d_Distance) < 100 && n_SpeedRatio >= 10)
			//2013,1127//2016.0330
// 			if(abs(dCurr[1] - d_Distance) < 100 && n_SpeedRatio >= 10)
// 			{
// 				d_SpeedVel = d_SpeedVel*0.5;
// 			}
			nMotModex = 0;
			dRet = XYDistance(0.0, dCurr[1], 0.0, d_Distance);
...
// 			}
	...
	else if(n_Axis == MOTOR_ROBOT_Z)
	{
		double dCurr;
		dCurr = COMI.Get_MotCurrentPos(MOTOR_ROBOT_Z);
		if(abs(dCurr - d_Distance) < 15 && n_SpeedRatio >= 10)
		{
			d_SpeedVel = d_SpeedVel*0.3;
		}
	}
	//////////////////////////////////////////////////////////////////////////
...
//   			st_motor[st_linearmot[n_MapIndex].lp_AxisNum[i]].d_spd_vel[2]);
	...
	if(n_MapIndex == M_ROBOT_XY_INDEX)
	{
		...
		dCurr[1] = COMI.Get_MotCurrentPos(MOTOR_ROBOT_Y);
		//2016.0330
// 		if(abs(dCurr[0] - dp_PosList[0]) < 200. && dp_SpdRatio[0] >= 10)
// 		{
// 			nMotMode[0] = 1;
// // 			md_spd_vel[MOTOR_ROBOT_X][0] = md_spd_vel[MOTOR_ROBOT_X][0]/2;
// 		}
// 		if(abs(dCurr[1] - dp_PosList[1]) < 200. && dp_SpdRatio[1] >= 10)
// 		{
// 			nMotMode[1] = 1;
// // 			md_spd_vel[MOTOR_ROBOT_Y][0] = md_spd_vel[MOTOR_ROBOT_Y][0]/2;
// 		}
		double dRet = XYDistance(dCurr[0], dCurr[1], dp_PosList[0], dp_PosList[1]);
		double dRetX = XYDistance(dCurr[0], 0, dp_PosList[0], 0);
		double dRetY = XYDistance(0, dCurr[1], 0, dp_PosList[1]);
		if( dRet < 200 )
		{
			nMotModex = 1;
			nMotModexY = 1;
...
// 		if( dRetX < 200 && dRetY > 200) nMotModex = 1;
		
// 		if	   ( dRet < 200 )  nMotModex = 1;
// 		else if( dRet < 500 )  nMotModex = 2;
// 		else if( dRet < 500 )  nMotModex = 3;
// 		else if( dRet < 1100 ) nMotModex = 4;
// 		else				   nMotModex = 0;
	}
	//////////////////////////////////////////////////////////////////////////
	//2016.0330
// 	if(nMotMode[0] == 1 && nMotMode[1] == 1) nMotModex = 1;
// 	else									 nMotModex = 0;
// 	if(nMotModex == 1)
// 	{
// 		nRet = Set_MotSpeed(MOT_SPD_CM_CTL, mp_axisnum[n_MapIndex][0], cmSMODE_S, 
// 			md_spd_vel[mp_axisnum[n_MapIndex][0]][0]*0.5, md_spd_vel[mp_axisnum[n_MapIndex][0]][1], md_spd_vel[mp_axisnum[n_MapIndex][0]][2]);  
// 	}
// 	else
// 	{
// 		nRet = Set_MotSpeed(MOT_SPD_CM_CTL, mp_axisnum[n_MapIndex][0], cmSMODE_S, 
...
// 	}
	...
	{
		nRet = Set_MotSpeed(MOT_SPD_CM_CTL, mp_axisnum[n_MapIndex][0], cmSMODE_S, 
			md_spd_vel[mp_axisnum[n_MapIndex][0]][0]*0.5, md_spd_vel[mp_axisnum[n_MapIndex][0]][1]*0.4, md_spd_vel[mp_axisnum[n_MapIndex][0]][2]*0.4);  
	}
	else if(nMotModex == 2)
	{
		nRet = Set_MotSpeed(MOT_SPD_CM_CTL, mp_axisnum[n_MapIndex][0], cmSMODE_S, 
			md_spd_vel[mp_axisnum[n_MapIndex][0]][0]*0.5, md_spd_vel[mp_axisnum[n_MapIndex][0]][1]*0.5, md_spd_vel[mp_axisnum[n_MapIndex][0]][2]*0.5);  
	}
	else if(nMotModex == 3)
	{
		nRet = Set_MotSpeed(MOT_SPD_CM_CTL, mp_axisnum[n_MapIndex][0], cmSMODE_S, 
			md_spd_vel[mp_axisnum[n_MapIndex][0]][0]*0.85, md_spd_vel[mp_axisnum[n_MapIndex][0]][1]*0.6, md_spd_vel[mp_axisnum[n_MapIndex][0]][2]*0.7);  
	}
	else if(nMotModex == 4)
	{
		nRet = Set_MotSpeed(MOT_SPD_CM_CTL, mp_axisnum[n_MapIndex][0], cmSMODE_S, 
			md_spd_vel[mp_axisnum[n_MapIndex][0]][0]*0.85, md_spd_vel[mp_axisnum[n_MapIndex][0]][1]*0.85, md_spd_vel[mp_axisnum[n_MapIndex][0]][2]*0.85);  
	}
	else if(nMotModex == 5)
	{
		nRet = Set_MotSpeed(MOT_SPD_CM_CTL, mp_axisnum[n_MapIndex][0], cmSMODE_S, 
			md_spd_vel[mp_axisnum[n_MapIndex][0]][0]*0.3, md_spd_vel[mp_axisnum[n_MapIndex][0]][1]*0.3, md_spd_vel[mp_axisnum[n_MapIndex][0]][2]*0.4);  
	}
	else
	{
		nRet = Set_MotSpeed(MOT_SPD_CM_CTL, mp_axisnum[n_MapIndex][0], cmSMODE_S, 
...
// 	if(nMotModex == 1)
// 	{
// 		nRet = Set_MotSpeed(MOT_SPD_CM_CTL, mp_axisnum[n_MapIndex][1], cmSMODE_S, 
// 			md_spd_vel[mp_axisnum[n_MapIndex][1]][0]*0.5, md_spd_vel[mp_axisnum[n_MapIndex][1]][1], md_spd_vel[mp_axisnum[n_MapIndex][1]][2]);  
// 	}
// 	else
// 	{
// 		nRet = Set_MotSpeed(MOT_SPD_CM_CTL, mp_axisnum[n_MapIndex][1], cmSMODE_S, 
...
// 	}
	...
	{
		nRet = Set_MotSpeed(MOT_SPD_CM_CTL, mp_axisnum[n_MapIndex][1], cmSMODE_S, 
			md_spd_vel[mp_axisnum[n_MapIndex][1]][0]*0.5, md_spd_vel[mp_axisnum[n_MapIndex][1]][1]*0.4, md_spd_vel[mp_axisnum[n_MapIndex][1]][2]*0.4);  
	}
	else if(nMotModexY == 2)
	{
		nRet = Set_MotSpeed(MOT_SPD_CM_CTL, mp_axisnum[n_MapIndex][1], cmSMODE_S, 
			md_spd_vel[mp_axisnum[n_MapIndex][1]][0]*0.5, md_spd_vel[mp_axisnum[n_MapIndex][1]][1]*0.5, md_spd_vel[mp_axisnum[n_MapIndex][1]][2]*0.5);  
	}
	else if(nMotModexY == 3)
	{
		nRet = Set_MotSpeed(MOT_SPD_CM_CTL, mp_axisnum[n_MapIndex][1], cmSMODE_S, 
			md_spd_vel[mp_axisnum[n_MapIndex][1]][0]*0.85, md_spd_vel[mp_axisnum[n_MapIndex][1]][1]*0.6, md_spd_vel[mp_axisnum[n_MapIndex][1]][2]*0.7);  
	}
	else if(nMotModexY == 4)
	{
		nRet = Set_MotSpeed(MOT_SPD_CM_CTL, mp_axisnum[n_MapIndex][1], cmSMODE_S, 
			md_spd_vel[mp_axisnum[n_MapIndex][1]][0]*0.85, md_spd_vel[mp_axisnum[n_MapIndex][1]][1]*0.85, md_spd_vel[mp_axisnum[n_MapIndex][1]][2]*0.85);  
	}
	else if(nMotModexY == 5)
	{
		nRet = Set_MotSpeed(MOT_SPD_CM_CTL, mp_axisnum[n_MapIndex][1], cmSMODE_S, 
			md_spd_vel[mp_axisnum[n_MapIndex][1]][0]*0.3, md_spd_vel[mp_axisnum[n_MapIndex][1]][1]*0.3, md_spd_vel[mp_axisnum[n_MapIndex][1]][2]*0.4);  
	}
	else
	{
		nRet = Set_MotSpeed(MOT_SPD_CM_CTL, mp_axisnum[n_MapIndex][1], cmSMODE_S, 
	...
	if (nRet == BD_ERROR)
	{
		...
		//2013,0405
		CTL_Lib.Alarm_Error_Occurrence(3015, CTL_dWARNING, mc_alarmcode);
		return BD_ERROR;
	}
...
// 	dp_SpdRatio[0] = dp_SpdRatio[0] * (double)((mn_runspeed_rate / 100.)); //james test 090311
	...
	//ybs 2012,1128 //2016.0330
// 	if(nMotModex == 1)
// 	{
// 		dp_SpdRatio[0] = 0.8 * dp_SpdRatio[0] * (double)((mn_runspeed_rate / 100.)); //james test 090311
// 	}
// 	else
// 	{
// 		dp_SpdRatio[0] = dp_SpdRatio[0] * (double)((mn_runspeed_rate / 100.)); //james test 090311
// 	}
	...
	nRet = cmmIxSetSpeedPattern(n_MapIndex, cmFALSE, cmSMODE_S, dp_SpdRatio[0], dp_SpdRatio[1], dp_SpdRatio[2]);
  	...
  	if (nRet != cmERR_NONE)
	{
		...
		//2013,0405
		CTL_Lib.Alarm_Error_Occurrence(3016, CTL_dWARNING, mc_alarmcode);
		return BD_ERROR;
	}
...
int CComizoaPublic::Check_LinearMove(int n_MapIndex, double *dp_PosList) 
{
	...
	for(i=0; i<ml_axiscnt[n_MapIndex]; i++)
	{
		...
		if( nRet == CTL_DOOROPEN)//2016.0330
		{
			cmmIxStop(n_MapIndex, FALSE, TRUE);
			return BD_ERROR;
		}
		if (nRet == BD_ERROR)
		{
			cmmIxStop(n_MapIndex, FALSE, TRUE);//2014,0730
...
//	int    nSafetyCheck = 0, Ret, i=0;
	...
	//2016.0330
	if( l_AxisCnt > 2 || lp_AxisNum[0] != MOTOR_ROBOT_X || lp_AxisNum[1] != MOTOR_ROBOT_Y)
	{
		if (st_handler.cwnd_list != NULL)  // 리스트 바 화면 존재
		{
			sprintf(st_msg.c_abnormal_msg, "Multimove)  AxisCnt:[%d] AxisNum[0]:[%d] AxisNum[1]:[%d]", l_AxisCnt, lp_AxisNum[0], lp_AxisNum[1]);
	...
	for(i=0; i<l_AxisCnt; i++)
	{
		...
		dTargetPos[i] = dp_PosList[i];//2016.0330
	}
...
// 	}
	...
	if(lp_AxisNum[0] == MOTOR_ROBOT_X )
	{
		dCurr[0] = COMI.Get_MotCurrentPos(MOTOR_ROBOT_X);
		dCurr[1] = COMI.Get_MotCurrentPos(MOTOR_ROBOT_Y);
		dRet = XYDistance(dCurr[0], dCurr[1], dp_PosList[0], dp_PosList[1]);
...
// 		else if( dRet < 1100 ) nMotModex = 4;
		...
		if(nMotModex == 1)
		{
			nRet = Set_MotSpeed(MOT_SPD_CM_CTL, lp_AxisNum[0], cmSMODE_S, md_spd_vel[lp_AxisNum[0]][0]*0.5, md_spd_vel[lp_AxisNum[0]][1]*0.4, md_spd_vel[lp_AxisNum[0]][2]*0.4);
		}
		else if(nMotModex == 2)
		{
			nRet = Set_MotSpeed(MOT_SPD_CM_CTL, lp_AxisNum[0], cmSMODE_S, md_spd_vel[lp_AxisNum[0]][0]*0.5, md_spd_vel[lp_AxisNum[0]][1]*0.5, md_spd_vel[lp_AxisNum[0]][2]*0.5);
		}
		else if(nMotModex == 3)
		{
			nRet = Set_MotSpeed(MOT_SPD_CM_CTL, lp_AxisNum[0], cmSMODE_S, md_spd_vel[lp_AxisNum[0]][0]*0.85, md_spd_vel[lp_AxisNum[0]][1]*0.6, md_spd_vel[lp_AxisNum[0]][2]*0.6);
		}
		else if(nMotModex == 4)
		{
			nRet = Set_MotSpeed(MOT_SPD_CM_CTL, lp_AxisNum[0], cmSMODE_S, md_spd_vel[lp_AxisNum[0]][0]*0.85, md_spd_vel[lp_AxisNum[0]][1]*0.85, md_spd_vel[lp_AxisNum[0]][2]*0.85);
		}
		else
		{
			nRet = Set_MotSpeed(MOT_SPD_CM_CTL, lp_AxisNum[0], cmSMODE_S, md_spd_vel[lp_AxisNum[0]][0], md_spd_vel[lp_AxisNum[0]][1], md_spd_vel[lp_AxisNum[0]][2]);			
		}
	}
	else
	{
	...
	{//010002 E A "Motor Move Setting Error01 motor)."
		...
		if (mn_errormsg_debug_mode)
		{
			sprintf(mc_normal_msg, "[axis=%d] multi move speed return error", lp_AxisNum[0]);
			Debug_File_Create(0, mc_normal_msg);
		}
		return BD_ERROR;
	}
	Set_MotSpeed_Ratio(lp_AxisNum[0], cmSMODE_T, dp_SpdRatio[0], dp_SpdRatio[1], dp_SpdRatio[2]);//V_0.1.4 090726 cmSMODE_KEEP 


	nMotModex = 0;
	if(lp_AxisNum[1] == MOTOR_ROBOT_Y)
	{
		dCurr[0] = COMI.Get_MotCurrentPos(MOTOR_ROBOT_X);
		dCurr[1] = COMI.Get_MotCurrentPos(MOTOR_ROBOT_Y);
		dRet = XYDistance(dCurr[0], dCurr[1], dp_PosList[0], dp_PosList[1]);
...
// 		else if( dRet < 1100 ) nMotModex = 4;
		...
		if(nMotModex == 1)
		{
			nRet = Set_MotSpeed(MOT_SPD_CM_CTL, lp_AxisNum[1], cmSMODE_S, md_spd_vel[lp_AxisNum[1]][0]*0.5, md_spd_vel[lp_AxisNum[1]][1]*0.4, md_spd_vel[lp_AxisNum[1]][2]*0.4);
		}
		else if(nMotModex == 2)
		{
			nRet = Set_MotSpeed(MOT_SPD_CM_CTL, lp_AxisNum[1], cmSMODE_S, md_spd_vel[lp_AxisNum[1]][0]*0.5, md_spd_vel[lp_AxisNum[1]][1]*0.5, md_spd_vel[lp_AxisNum[1]][2]*0.5);
		}
		else if(nMotModex == 3)
		{
			nRet = Set_MotSpeed(MOT_SPD_CM_CTL, lp_AxisNum[1], cmSMODE_S, md_spd_vel[lp_AxisNum[1]][0]*0.85, md_spd_vel[lp_AxisNum[1]][1]*0.6, md_spd_vel[lp_AxisNum[1]][2]*0.6);
		}
		else if(nMotModex == 4)
		{
			nRet = Set_MotSpeed(MOT_SPD_CM_CTL, lp_AxisNum[1], cmSMODE_S, md_spd_vel[lp_AxisNum[1]][0]*0.85, md_spd_vel[lp_AxisNum[1]][1]*0.85, md_spd_vel[lp_AxisNum[1]][2]*0.85);
		}
		else
		{
			nRet = Set_MotSpeed(MOT_SPD_CM_CTL, lp_AxisNum[1], cmSMODE_S, md_spd_vel[lp_AxisNum[1]][0], md_spd_vel[lp_AxisNum[1]][1], md_spd_vel[lp_AxisNum[1]][2]);			
		}
	}
	else
	{
	...
	if (nRet == BD_ERROR)
	{//010002 E A "Motor Move Setting Error01 motor)."
		sprintf(mc_alarmcode, "%02d0002", lp_AxisNum[1]);
		if (mn_errormsg_debug_mode)
		{
			sprintf(mc_normal_msg, "[axis=%d] multi move speed return error", lp_AxisNum[1]);
			Debug_File_Create(0, mc_normal_msg);
		}
		return BD_ERROR;
	}
	Set_MotSpeed_Ratio(lp_AxisNum[1], cmSMODE_T, dp_SpdRatio[0], dp_SpdRatio[1], dp_SpdRatio[2]);//V_0.1.4 090726 cmSMODE_KEEP 


	nRet_l = CTL_Lib.Motor_LinearSafety(M_ROBOT_XY_INDEX, dp_PosList);
	if( nRet_l == BD_ERROR)
	{
		for(i=0; i<l_AxisCnt; i++)
		{
...
int CComizoaPublic::HomeCheck_Mot(int n_Axis, int n_TimeOut) 
{
	...
	{
		nFuncRet = BD_PROCEED;
		g_sm.SetData( (SM_TYPE)(SMT_MOTOR_BIN_1_Z_TARGET + n_Axis), "0");
		if( g_sm.GetData( (SM_TYPE)(SMT_MOTOR_BIN_1_Z_POSITION + n_Axis) ) == "0" )
		{
			nFuncRet = BD_GOOD;
		}
		return nFuncRet;
...
//2016.0330
double CComizoaPublic::XYDistance(double xStart, double yStart, double xEnd, double yEnd)
{
	
	double dRet = 0.0;
	
	dRet = sqrt( pow ( (xEnd - xStart), 2 ) + pow ( ( yEnd - yStart), 2 ) );
