...
void CDialog_Pass_Check::OnPassChk_Level_Set()
{
	...
	else if (st_handler.mstr_pass_level == "MAX_COUNT")		mn_pass_level = 3;
}
...
void CDialog_Pass_Check::OnPassChk_Pass_Data_Load()
{
	...
	mcls_pass_chk.OnPassWord_Load("MAXCOUNT");			// 레벨에 따른 암호 정보 로딩 함수
	
	if ((st_msg.mstr_password).IsEmpty())  m_msg_pass_chk.SetWindowText(_T("Haven't Password"));
	else	mstr_level3_chk = st_msg.mstr_password;	// 전역 변수에 저장된 암호 정보 클래스 변수에 설정 
...
void CDialog_Pass_Check::OnOK() 
{
	...
	case 3:
		if (mstr_level3_chk == mstr_pass_chk)						// 암호가 Level1일때...
		{		
		}
		else
		{
