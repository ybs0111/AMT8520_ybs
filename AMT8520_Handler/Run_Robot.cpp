...
// 					pMdl_4 = &g_Tray.GetTray( nTrayIdx ).GetModule( (job.iPosFirst - st_basic.n_tray_x + 1) % MPOS_DIVISION );
				...
				if( nTrayIdx > -1 && st_handler.mn_xgem_mode == CTL_YES )
				{
					if( job.iRobotFirst != -1 )
					{
						pMdl_1->SetTrayNum(g_Tray.GetTray(nTrayIdx).GetBufferNo());
						nTestCnt = pMdl_1->GetTestCnt();
						if( nTestCnt > 0 )//retest pickup
						{
							g_client_xgem.SetDVCRetestPickStart( pMdl_1->GetSerial() );
						}
						else
						{
							g_client_xgem.SetLdRbtPickupDVCEnd( g_Tray.GetTray(nTrayIdx).GetBufferNo(), pMdl_1->GetSerial() );
						}
					}
					if( job.iRobotSecond != -1 )
					{
						pMdl_2->SetTrayNum(g_Tray.GetTray(nTrayIdx).GetBufferNo());
						nTestCnt = pMdl_2->GetTestCnt();
						if( nTestCnt > 0 )//retest pickup
						{
							g_client_xgem.SetDVCRetestPickStart( pMdl_2->GetSerial() );
						}
						else
						{
							g_client_xgem.SetLdRbtPickupDVCEnd( g_Tray.GetTray(nTrayIdx).GetBufferNo(), pMdl_2->GetSerial() );
						}
					}
					if( job.iRobotThird != -1 )
					{
						pMdl_3->SetTrayNum(g_Tray.GetTray(nTrayIdx).GetBufferNo());
						nTestCnt = pMdl_3->GetTestCnt();
						if( nTestCnt > 0 )//retest pickup
						{
							g_client_xgem.SetDVCRetestPickStart( pMdl_3->GetSerial() );
						}
						else
						{
							g_client_xgem.SetLdRbtPickupDVCEnd( g_Tray.GetTray(nTrayIdx).GetBufferNo(), pMdl_3->GetSerial() );
						}
					}
					if( job.iRobotFourth != -1 )
					{
						pMdl_4->SetTrayNum(g_Tray.GetTray(nTrayIdx).GetBufferNo());
						nTestCnt = pMdl_4->GetTestCnt();
						if( nTestCnt > 0 )//retest pickup
						{
							g_client_xgem.SetDVCRetestPickStart( pMdl_4->GetSerial() );
						}
						else
						{
							g_client_xgem.SetLdRbtPickupDVCEnd( g_Tray.GetTray(nTrayIdx).GetBufferNo(), pMdl_4->GetSerial() );
						}
					}
				}
			...
			else if( job.GetPosByTarget() == MPOS_INDEX_BIN2 )
			{
				...
				if(nTrayIdx < 0 || job.iPosFirst < 0 || job.iPosFirst > 3 )
				{
					if (st_handler.cwnd_list != NULL)  // 리스트 바 화면 존재
					{
						m_str_msg.Format("PICK_FINISH2) TrayIdx :[%d] job.iPosFirst:[%d]", nTrayIdx, job.iPosFirst);
						Func.On_LogFile_Add(99, m_str_msg);
					}
				}
				//////////////////////////////////////////////////////////////////////////
...
// 					pMdl_4 = &g_Tray.GetTray( nTrayIdx ).GetModule( (job.iPosFirst - st_basic.n_tray_x + 1) % MPOS_DIVISION );
			...
			else if( job.GetPosByTarget() == MPOS_INDEX_BIN3 )//ybs
			{
				...
				if(nTrayIdx < 0 || job.iPosFirst < 0 || job.iPosFirst > 3 )
				{
					if (st_handler.cwnd_list != NULL)  // 리스트 바 화면 존재
					{
						m_str_msg.Format("PICK_FINISH3) TrayIdx :[%d] job.iPosFirst:[%d]", nTrayIdx, job.iPosFirst);
						Func.On_LogFile_Add(99, m_str_msg);
					}
				}
				//////////////////////////////////////////////////////////////////////////
			...
			if( job.GetPosByTarget() == MPOS_INDEX_TESTSITE )
			{
				...
				if(st_basic.mn_mode_xgem == CTL_YES)
				{
					...
					if( st_handler.mn_xgem_mode == CTL_YES )
					{
						if( pMdl_1 != NULL )
						{
							g_client_xgem.SetLdRbtPickupDVCEnd( pMdl_1->GetTrayNum(), pMdl_1->GetSerial() );
						}
						if( pMdl_2 != NULL )
						{
							g_client_xgem.SetLdRbtPickupDVCEnd( pMdl_2->GetTrayNum(), pMdl_2->GetSerial() );
						}
					}
					else
					{
