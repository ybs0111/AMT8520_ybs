...
void CScreen_Motor::OnCellClick(WPARAM wParam, LPARAM lParam)
{
	...
	{
		str_file.Format("C:\\AMT8520\\Bmp\\%s", st_part.str_part_info[lpcc->Col-1]);
...
{
	for( int iM=0; iM<MOTOR_COUNT; iM++ )
	{
		for( int i=0; i< MAX_POS; i++ )
		{
			...
			CString strVal;
			strVal.Format( "%0.3f", st_motor[iM].d_pos[i] );
...
void CScreen_Motor::OnBtnTeachRead() 
{
	for( int iM=0; iM<MOTOR_COUNT; iM++ )
	{
		for( int i=0; i< MAX_POS; i++ )
		{
			...
			st_motor[iM].d_pos[i] = atof( AMTRegistry::RegReadString( REG_KEY_TEACH, strValName, 0 ).c_str() );
		}
	}
}
